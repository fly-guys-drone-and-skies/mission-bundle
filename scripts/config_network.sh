...
# To see a list of network interfaces interfaces, run `ip a`.
# This configuration must occur every time a SkyController is plugged in - the rules will not persist.
# Cleanup is not required if the device is unplugged.

# Setup Usage Examples:
# ./config_network.sh setup <interface> <unique ip to use> <unique rule number>
# ./config_network.sh setup ens35u1 192.168.54.1 201
# ./config_network.sh setup ens35u2 192.168.55.1 202
# Cleanup usage:
# ./config_network.sh cleanup ens35u1 192.168.54.1 201
# ./config_network.sh cleanup ens35u2 192.168.55.1 202
